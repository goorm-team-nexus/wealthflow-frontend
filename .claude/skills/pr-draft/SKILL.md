---
name: pr-draft
description: Draft a GitHub PR title and body from the current branch using this project's PR template. Use when the user invokes /pr-draft or asks for a PR draft.
allowed-tools: Bash(git branch *) Bash(git rev-parse *) Bash(git status *) Bash(git fetch *) Bash(git diff *) Bash(git log *)
---

# PR Draft

Follow `.agents/workflows/pr-draft.md` exactly.

Treat any text after `/pr-draft` as invocation input for the shared workflow.

Do not create a PR, commit, push, merge, or edit files.
